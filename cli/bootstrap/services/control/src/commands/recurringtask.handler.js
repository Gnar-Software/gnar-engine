 ...
 */
commands.register('controlService.getManyRecurringTasks', async ({ pageSize, pageNum, filters }) => {
    return await recurringTask.getAll({ pageSize, pageNum, filters });
});
...
commands.register('controlService.scheduleFromRecurringTasks', async () => {
    ...
    const recurringTasksToSchedule = await recurringTask.getAll({
        filters: {
            nextRunAt: { operator: 'lte', value: new Date() }
        },
        pageSize: 9999,
        pageNum: 1
    });
    ...
    for (const recurringTaskObj of recurringTasksToSchedule.data) {
        ...
        try {
            ...
            // calculate when we need to schedule the next task for:
            // use the latest scheduled task's scheduled at,
            // or the recurring task's nextRunAt if there are no future tasks
            let cursorDate = new Date(recurringTaskObj.nextRunAt);
            ...
            const nextInterval = utils.cronExpressionParser.parse(recurringTaskObj.cronExpression, {
                currentDate: new Date(recurringTaskObj.nextRunAt)
            });
