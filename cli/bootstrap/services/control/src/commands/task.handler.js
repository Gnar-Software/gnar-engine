...
commands.register('controlService.handleTaskBatch', async (status = null) => {
    ...
    try {
        const scheduledTasks = await task.getDueTasksForBatch({
            status: status
        });
        const processingRecurringTaskIds = scheduledTasks
            .filter(taskObj => processingTaskIds.includes(taskObj.id) && taskObj.recurringTaskId)
            .map(taskObj => taskObj.recurringTaskId);
        const recurringTaskIds = [];
        const taskIds = [];

        // Only queue tasks that are not blocked by an active in-memory task run.
        scheduledTasks.forEach(taskObj => {
            if (processingTaskIds.includes(taskObj.id)) {
                return;
            }

            if (!taskObj.recurringTaskId) {
                taskIds.push(taskObj.id);
                return;
            }

            if (processingRecurringTaskIds.includes(taskObj.recurringTaskId)) {
                return;
            }

            if (recurringTaskIds.includes(taskObj.recurringTaskId)) {
                return;
            }

            recurringTaskIds.push(taskObj.recurringTaskId);
            taskIds.push(taskObj.id);
        });

        // Reserve selected task IDs before the fetch so overlapping batches in this process skip them.
        reservedTaskIds = taskIds;
        reservedTaskIds.forEach(taskId => {
            if (!processingTaskIds.includes(taskId)) {
                processingTaskIds.push(taskId);
            }
        });
        ...
        // Release any reserved IDs that were not fetched, for example if their status changed meanwhile.
        const fetchedTaskIds = tasks.map(taskObj => taskObj.id);
        reservedTaskIds
            .filter(taskId => !fetchedTaskIds.includes(taskId))
            .forEach(taskId => {
                const processingTaskIndex = processingTaskIds.indexOf(taskId);

                if (processingTaskIndex !== -1) {
                    processingTaskIds.splice(processingTaskIndex, 1);
                }
            });
    } catch (error) {
        reservedTaskIds.forEach(taskId => {
    ...
    const taskPromises = tasks.map(taskObj => (
        (async () => {
            ...
            try {
                ...
                const processingTaskIndex = processingTaskIds.indexOf(taskObj.id);
...
commands.register('controlService.handleTask', async ({ task: taskObj }) => {
    try {
        ...
        // Execute task
        try {
            const response = await commands.execute(taskObj.handler, taskObj.payload, { timeout: 600000 });

            // if there are errors (these are for runtime errors - should not be used for retrying)
            if (response && typeof response === 'object' && (response.error || response.status === 'failed')) {
 ...
 * Get tasks by status
 *
 * @param {string|Object} params - Task status or params
 */
commands.register('controlService.getTasksByStatus', async (params) => {
    try {
        const status = typeof params === 'string' ? params : params?.status;
        const orderDirection = typeof params === 'object' ? params.orderDirection : 'ASC';
 ...
 * Delete task
 */
commands.register('controlService.deleteTask', async ({ id }) => {
    try {
        // Check the task exists before attempting to delete it.
        const existingTask = await task.getById({ id: id });
        if (!existingTask) {
            throw new Error('Task not found');
        }

        // Delete the single task record by id.
